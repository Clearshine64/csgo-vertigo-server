...
const open_rank = async (winscore) => {
    try {
        ...
        if (Math.floor((Math.random() * 2))) {
            scores.fscore = Math.floor(winscore/2)+2; //competitive: 9
            ...
            scores.fscore = 0;
            scores.sscore = Math.floor(winscore/2)+2; //competitive: 9
        }
...
const level = async (winscore) => {
    try {
        let scores = {};
        scores.fscore = Math.floor((winscore - 2)/2) + 3; // 9
        scores.sscore = Math.floor((winscore - 2)/2) + 3; // 9
        return scores;
    } catch (err) {
        console.log(err);
        throw err;
