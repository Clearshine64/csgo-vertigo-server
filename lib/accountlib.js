...
const setStatusFlagAndDesc = async (id, flag, desc) => {
    try {
        ...
        if(flag == "processed")
        {
            //set profile_after getinfo 0;
            let profile = await getProfileAfter(id);
            profile.getInfo = 0;    //this flag is used to check whether to get info from steam in final

            await setProfileAfter(id, profile);
        }
