...
if (process.env.NODE_ENV == 'test') {
    ...
    timeconfig = {
        beforeInvited: 120000,
        beforeMatchid: 600000,
        beforeMatchStart: 120000,
        duringAccept: 60000,
        beforeMatching: 300000,
        beforeUpload: 60000
    };
...
const check_only_lose = async (mode, groupid) => {
    try {
        ...
        if (loseDB[groupid] == condition.condition) {
            flag = true;
...
const check_level = async (mode, groupid) => {
    try {
        ...
        if (levelDB[groupid] == condition.condition) {
            flag = true;
...
const matchManage = async (json, vbclient) => {
    try {
        ...
        switch (json.action) {
            case "csgo_started":
                await sendMessage("invite", { groupid: json.groupid }, vbclient, "notprocessed", "error occured in search match", timeconfig.beforeMatchid);
                break;
            ...
            case "first_clicker_done":
                {
                    ...
                    await sendMessage("clicker_second", { groupid: opponentId }, matchids[mode][opponentId]["vbclient"], "notprocessed", "error occured in research match", 0);
                }
                break;
            ...
            case "uploading":
                ...
                // all false means accept match
                {
                    ...
                    for (let account of json.accounts) {
                        ...
                        profile.ranking.wins = account.wins || 0;
                        profile.ranking.rank_id = account.ranking || 0;
                        profile.player_cur_xp = account.xppts || 0;
                        profile.player_level = account.level || 0;
                        console.log(profile);
                        */
                        let profile = await AccountLib.getProfileAfter(account._id);
                        profile["ranking"] = profile["ranking"] || {};
                    ...
                    if (!flag) {
                        await sendMessage("invite", { groupid: json.groupid }, vbclient, "notprocessed", "error occured in invite", timeconfig.beforeMatchid);
                    }
                }
                break;
            ...
            case "notconnected":
                {
                    let mode = await GroupLib.getMatchMode(json.groupid);
                    await setAccountsFlagDescInGroup(mode, "notprocessed", "connection is failed to csgo server");
                }
                break;
        }
    } catch (err) {
