            ...
            case "first_clicker_done":
                {
                    ...
                    await sendMessage("clicker_second", { groupid: opponentId }, matchids[mode][opponentId]["vbclient"], "notprocessed", "error occured in match", 0);
                }
                break;
                ...
                // all false means accept match
                {
                    ...
                    for (let account of json.accounts) {
                        ...
                        */
                        ...
                        if(account.xppts && account.xppts != 0 && account.xppts > profile.player_cur_xp)
                            profile.player_cur_xp = account.xppts;
                        
                        if(account.level && account.level != 0 && account.level > profile.player_level)
